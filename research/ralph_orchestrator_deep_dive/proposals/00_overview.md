...
Proposals for enriching the harness design docs with patterns identified in the ralph-orchestrator and Ralph Wiggum deep dives. Each proposal is self-contained and targets a specific harness doc. No existing files are modified — these are proposed additions for review.
...
Each proposal addresses a gap where ralph-orchestrator or bare Ralph has a mature, tested pattern that the harness lacks:

1. **Ralph Ecosystem Positioning** — The harness docs don't mention Ralph anywhere. Given that ralph-orchestrator is the closest community implementation (2,088 stars, same core insight of context rotation), documenting the relationship clarifies the harness's unique value proposition.

2. **Stagnation Detection** — Ralph's three-detector model (stale, thrashing, consecutive failures) is the most mature stagnation detection in the ecosystem. The harness lists this as "planned" but has no design. Ralph provides a concrete, tested model to adapt.

3. **Inter-Session Validation** — Ralph's backpressure gates (validation between iterations) catch regressions early. The harness only validates after the final session. For multi-task workflows, early failure detection reduces wasted compute.
...
5. **LEARNINGS.md Seed File** — Formalizes operational knowledge transfer between sessions. Ralph's scratchpad serves this purpose; the harness has seed files but no pattern for knowledge (vs. code) transfer.

6. **JSONL Event Stream** — Ralph's event bus provides more granularity than the harness's before/after snapshot model. A continuous event log enables debugging, trend analysis, and CI/CD integration.
...
|---------|-------------|--------|
| **Hat system** (persona routing) | The harness monitors agents, it doesn't orchestrate personas. Adding persona routing would conflate monitoring with orchestration. | [synthesis](../00_synthesis.md) §4.3 |
| **PTY execution** | PTY introduces terminal escape codes that complicate stream parsing. Piped stdout is more reliable for the harness's monitoring mission. | [synthesis](../00_synthesis.md) §4.4 |
...
| **Drop context pressure monitoring** | Ralph's "short iterations avoid the problem" is a heuristic, not a solution. The zone model is the harness's core value. | [synthesis](../00_synthesis.md) §4.1 |
| **Scratchpad (16K FIFO)** | The harness's seed files + LEARNINGS.md achieves the same goal with more operator control. | [synthesis](../00_synthesis.md) §3.4 |
...
- Harness design docs: ARCHITECTURE.md, SESSIONS.md, TASKS.md, REPORTS.md
