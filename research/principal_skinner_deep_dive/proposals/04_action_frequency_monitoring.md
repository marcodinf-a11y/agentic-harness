...
Default thresholds (configurable in `rein.toml`):
...
The Principal Skinner deep dive identifies action-content analysis as Rein's primary safety gap ([synthesis](../00_synthesis.md) §4.4). Rein monitors context pressure (resource-based) but not action patterns (behavior-based). Principal Skinner proposes full behavioral circuit breakers; the critical analysis ([06_critical_analysis.md](../06_critical_analysis.md)) concludes this is over-engineering for local development.
...
4. **Bridges the gap.** Rein goes from knowing "the agent used 60% of context" to knowing "the agent used 60% of context AND ran 47 bash commands, 2 destructive operations, and 0 network calls."
...
- [Circuit Breakers](../03_circuit_breakers.md) — Section 4 (rein gap: no action-content analysis)
