...
2. **Commit uncommitted changes** — check `git status` for uncommitted work. If present: `git add -A && git commit -m "rein: uncommitted work captured at session end (termination_reason={termination_reason})"` in the sandbox. Runs on *any* termination reason (not just Rein-initiated kills) — defense-in-depth against agents that fail to commit. The distinctive `rein:` prefix marks this as Rein's safety commit, not the agent's deliberate work. The commit uses the agent's git identity (same `GIT_AUTHOR_*` / `GIT_COMMITTER_*` env vars set during sandbox setup — see [ARCHITECTURE.md — Agent Git Identity](ARCHITECTURE.md#agent-git-identity)), since the wrap-up is capturing the agent's uncommitted work.
...
6. **Track reload overhead** — each fresh session pays a cold-start tax to re-ingest the system prompt, task definition, seed files, and LEARNINGS.md. The `reload_tokens` counter in `NormalizedTokenUsage` tracks this per session. Cumulative `reload_overhead_pct` across sessions reveals how much of the total token budget goes to context setup vs. productive work. High overhead (> 30%) signals that task decomposition is too fine-grained or seed files are too large. See [TOKENS.md — Context Reload Cost](TOKENS.md#context-reload-cost).
