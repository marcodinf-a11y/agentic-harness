# Deep Dive Plan: Principal Skinner Harness Pattern

## Context for the Executing Agent

You are creating a deep dive research document for the **agentic harness** project — a context-pressure-aware orchestrator for AI coding agents (Claude Code, Codex CLI, Gemini CLI). The harness monitors how agents consume their context windows, intervenes when quality is at risk (via green/yellow/red zone thresholds), and ensures work is persisted before degradation sets in. It uses subprocess isolation, real-time token monitoring, and structured evaluation with binary scoring.

The harness project has already completed deep dives on:
...
- How the harness already implements (or doesn't implement) each Principal Skinner mechanism
- Transferable patterns worth adopting in the harness
...
- How this relates to the harness's existing safety model (subprocess isolation, sandbox, zone kills)
- Whether the harness has gaps that Principal Skinner addresses
...
- How this compares to the harness's subprocess isolation model
- Whether the harness should add tool-use interception or whether subprocess isolation is sufficient
...
- Comparison with the harness's zone-based intervention (yellow = graceful stop, red = kill)
- Whether the harness needs circuit breakers beyond zone thresholds
...
- How this compares to the harness's session reports (which already track agent identity per task)
- Whether the harness needs stronger attribution (e.g., agent-specific git config per session)
...
- Whether this is practical for the harness (cost, time, implementation complexity)
- Relationship to the harness's multi-run evaluation recommendation from the RLM deep dive
- Could the harness's evaluation framework be extended for adversarial simulation?
...
- What the harness already does better than Principal Skinner
- What Principal Skinner does better than the harness
...
4. **Context:** Read `ARCHITECTURE.md`, `SESSIONS.md`, `BRIEF.md` in this repo for the harness's existing safety model
5. **Background:** Search for OWASP Agentic Security guidelines, particularly ASI08 (Agentic Misalignment)
6. **Background:** Search for "agent safety harness" "tool use control" "agentic circuit breaker" for related work
...
- Compare against the harness's existing capabilities in every section
- Be adversarial: identify over-engineering, impractical proposals, and missing evidence
- Assess whether each Principal Skinner mechanism is worth the implementation cost for the harness
